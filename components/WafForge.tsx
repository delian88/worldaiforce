...
const WafForge: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (showTutorialVideo) {
      ...
      if (tutorialStep === 1) {
        const fullText = "Create an image of a futuristic city with AI integration.";
        let charIdx = 0;
        setTutorialTyping('');
  ...
  const forgeImage = async () => {
    ...
    try {
      ...
      const errorMsg = error.message.includes('429') ? 'API Quota Exhausted. Wait 60s.' : error.message;
      addLog(`> ERR: ${errorMsg}`);
  ...
  const forgeContent = async () => {
    ...
    try {
      ...
      setResult({ type: 'content', text: res.text });
  ...
  const forgeAudio = async () => {
    setLoading(true);
    setStatus('Synthesizing Vocal Synapse...');
    addLog('> AUDIO_FORGE_START');
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const res = await ai.models.generateContent({
      ...
      if (audioData) {
        setResult({ type: 'audio', audioData });
  ...
  const handleForge = () => {
    ...
    else if (activeTool === 'content') forgeContent();
    else if (activeTool === 'audio') forgeAudio();
  };

  const tutorialSteps = [
    { title: "Select Tool", desc: "Choose your output modality: Image, Content, Audio, or Video.", icon: <Monitor className="w-8 h-8" /> },
    ...
    { title: "Initiate Forge", desc: "Click the Forge button to start the synaptic synthesis protocol.", icon: <Zap className="w-8 h-8" /> },
      ...
      {showTutorialVideo && (
        <div className="absolute inset-0 z-[1000] bg-slate-950 flex flex-col p-6 md:p-12 animate-in fade-in duration-500 overflow-hidden">
           <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={() => setShowTutorialVideo(false)} className="p-3 bg-white/5 hover:bg-white/10 rounded-full text-slate-400"><X /></button>
          </div>
          ...
          <div className="flex-1 grid md:grid-cols-2 gap-12 items-center">
            <div className="relative aspect-video bg-black rounded-[3rem] border border-white/10 overflow-hidden shadow-3xl flex items-center justify-center">
               ...
               <div className="w-[85%] h-[80%] glass rounded-3xl border border-white/5 p-6 flex flex-col gap-4 relative z-0">
                  ...
                  <div className="flex-1 bg-white/5 rounded-2xl p-4 overflow-hidden relative">
                     {tutorialStep === 0 && <div className="absolute inset-0 flex items-center justify-center"><Cpu className="w-12 h-12 text-blue-400" /></div>}
            ...
            <div className="space-y-8">
               <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-black uppercase tracking-widest">PHRASE {tutorialStep + 1}</div>
               <div key={tutorialStep} className="animate-in slide-in-from-bottom-4 duration-700">
                  <h4 className="text-4xl md:text-6xl font-display font-bold text-white mb-6 flex items-center gap-6">{tutorialSteps[tutorialStep].icon}{tutorialSteps[tutorialStep].title}</h4>
                  <p className="text-slate-400 text-lg md:text-2xl font-light leading-relaxed">{tutorialSteps[tutorialStep].desc}</p>
               </div>
            </div>
          </div>
          <div className="mt-12 flex justify-center gap-6"><button onClick={() => setShowTutorialVideo(false)} className="px-12 py-5 bg-blue-600 text-white rounded-full font-black uppercase tracking-[0.4em] text-[10px] shadow-2xl">Return to terminal</button></div>
        </div>
      )}
          ...
          <div className="lg:w-1/2 space-y-8">
            <div className="relative">
               <textarea
                ...
                placeholder={`Describe your vision for World AI Force intelligence... (e.g., "A futuristic boy with his robot friend in a galaxy")`}
                ...
                <div className="w-full h-full p-6 overflow-y-auto custom-scrollbar">
                  <div className="space-y-8 animate-in fade-in duration-700">
                    <div className="flex items-center gap-4 text-blue-400 border-b border-white/10 pb-4">
                      ...
                      <h4 className="font-display font-bold text-xl uppercase tracking-tighter">Synaptic Output</h4>
                    </div>
                    
                    {result.type === 'image' && result.url && (
                       <img src={result.url} className="w-full rounded-3xl shadow-2xl border border-white/10" alt="Forged asset" />
                    )}

                    {result.type === 'content' && result.text && (
                       <div className="p-6 text-slate-300 font-light leading-relaxed whitespace-pre-wrap text-lg italic border-l-2 border-blue-500/30 pl-6">
                         {result.text}
                       </div>
                    )}
                    ...
                    {result.type === 'audio' && result.audioData && (
                      <div className="p-8 bg-blue-600/5 rounded-3xl border border-blue-500/20 flex items-center justify-between group">
                        ...
                        <button onClick={() => playForgedAudio(result.audioData!)} className="p-4 bg-blue-600 rounded-full text-white hover:scale-110 transition-transform">
                          {isPlaying ? <Loader2 className="w-4 h-4 animate-spin" /> : <PlayIcon className="w-4 h-4 fill-current" />}
                        </button>
                      </div>
                    )}
                  </div>
                  <button onClick={() => setResult(null)} className="absolute top-6 right-6 p-2 bg-red-600/20 rounded-full text-red-500 hover:bg-red-600/40 transition-colors"><X className="w-4 h-4" /></button>
                </div>
              )}
            </div>
          </div>
